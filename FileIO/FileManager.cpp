...
void osuFileOperations::readSingleFile(std::vector<osuBeatmap> *array, std::string file, ProgressTracker *pt)
{
	...
	bool backgroundIsNextLine = false; //is the background the next line? (used in the loop) (this is the autogenerated osu comment, //Background and Video events)
	...
	while (std::getline(fs, buffer))
	{
		...
		{
			std::string MusicLocation = fileOperations::rootDirectoryOfFile(file) + buffer.substr(buffer.find(":") + 1, string::npos);
			if (MusicLocation[0] == ' ')
			{
		...
		if (buffer.find("//Background and Video events"))
		{
			backgroundIsNextLine = true; //the next line is the background
		}

		if (backgroundIsNextLine) //background is found
		{
